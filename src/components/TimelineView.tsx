...
function getActionTone(actionType?: AgentAction['type']) {
  switch (actionType) {
    ...
    default:
      return {
        chip: 'bg-surface text-content-secondary border-border',
        dot: 'bg-muted',
      };
  }
}
  ...
  switch (stage) {
    ...
    default:
      return {
        ...
        chip: 'border-border bg-surface text-content-secondary',
      };
  }
}
    ...
    <div ref={cardRef} className="relative pb-3">
      ...
      <div className="relative flex gap-4">
        ...
        <div className="flex flex-col items-center">
          ...
          {index < totalItems - 1 && (
            <div className="w-0.5 flex-1 bg-gradient-to-b from-surface-hover to-surface mt-2" style={{ minHeight: '2rem' }} />
          )}
        </div>

        {/* Entry content */}
        <div className={`flex-1 min-w-0 ${entry.isForked ? 'opacity-40 grayscale-[0.5]' : ''}`}>
          <div className={`cyber-card overflow-hidden transition-all ${density === 'summary' ? 'p-4' : 'p-5'} ${entry.isForked ? 'border-dashed border-border bg-surface/10' : ''} ${entry.parsedAction?.type === 'TerminalCommand' && (entry.parsedAction as any).exitCode !== 0 && (entry.parsedAction as any).exitCode !== -1 ? 'cyber-card-error' : ''}`}>
            <div className={`flex items-start justify-between gap-2 ${density === 'summary' ? 'mb-2' : 'mb-3'}`}>
              <div className="flex items-center gap-2 min-w-0 flex-1">
                <span className="text-[10px] font-black uppercase tracking-widest text-muted">{roleLabel}</span>
                {entry.isForked && (
                  <span className="px-2 py-0.5 bg-surface text-content-secondary text-[9px] rounded-full shrink-0 border border-border uppercase font-black">
                    Ghost
                  </span>
                )}
              </div>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <h3 className="text-sm font-black text-content tracking-wide truncate">{summary.title}</h3>
                        {action && (
                      ...
                      <p className="text-sm text-content/90 leading-relaxed break-words">
                      ...
                      {groupedCount > 1 && (
                        <div className="mt-2 flex flex-wrap gap-2">
                          ...
                          {groupedCount > 3 && (
                            <span className="rounded-full border border-border bg-surface/80 px-2.5 py-1 text-[11px] leading-tight text-content-secondary">
                              +{groupedCount - 3} more
                            </span>
                          )}
                        </div>
                ...
                {density === 'summary' ? (
                  <div className="space-y-4">
                    {item.entries.map((groupEntry: any, groupIndex: number) => (
                      <div key={groupEntry.uuid || `${item.key}-${groupIndex}`} className="rounded-2xl border border-border/70 bg-background/30 p-4">
  ...
  if (!data || !data.entries) {
    return (
      <div className="p-10 text-center">
        <p className="text-content-secondary">No log data available</p>
      </div>
    );
  }
      ...
      <div className="flex-1 overflow-hidden pr-6 h-full flex flex-col" ref={containerRef}>
        <div className="mb-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-2 rounded-2xl border border-border bg-surface/60 p-1">
              <button
        ...
        {entriesWithKeys.length === 0 ? (
          <div className="flex-1 flex items-center justify-center bg-surface/20 rounded-3xl border-2 border-dashed border-border">
            <div className="text-center">
              <Search className="w-8 h-8 text-muted mx-auto mb-2" />
              <p className="text-muted text-sm font-bold">No matching log entries</p>
            </div>
          </div>
        ) : (
          density === 'summary' ? (
